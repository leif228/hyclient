...
public abstract class OverlayManager implements OnMarkerClickListener, OnPolylineClickListener {
    ...
    /**
     * 将所有Overlay 添加到地图上，返回当前clickedMarker
     */
    public Marker addToMap2(ShipCooordData curr) {
    	...
    	for (OverlayOptions option : mOverlayOptionList) {
    		...
    		if (curr!=null && (overlay instanceof Marker) && (curr.getShipName().equals(((Marker) overlay).getTitle()))) {
    			currMarker = (Marker) overlay;
            }
    ...
    /**自定义*/
    public void addOverlayOptionsToMap(OverlayOptions option){
    	mOverlayList.add(mBaiduMap.addOverlay(option));
    }
    
    public void removeOverlay(String shipName){
    	for(Overlay overlay : mOverlayList){
    		if (overlay!=null&&overlay instanceof Marker && shipName.equals(((Marker) overlay).getTitle())) {
                ((Marker) overlay).remove();;
            }else if(overlay!=null&&overlay instanceof Text && shipName.equals(((Text) overlay).getText())){
            	((Text) overlay).remove();
            }
    	}
    }
